...
import Mission from "../../pages/edit/Mission";

const Misison_list = () => {
  const MySwal = withReactContent(Swal);

  //sub stream
  const { data, error, refetch } = useQuery("misionData", async () => {
    const response = await axios.get(
      "https://yarnlink-server.onrender.com/api/mission",
      {
        mode: "cors",
      }
    );
              ...
              <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content" style={{ width: 700 }}>
                  <div className="modal-body">
                    <Mission data={row} refetch={refetch} />
                  </div>
                </div>
              </div>
            </div>
  ...
  const handleCategory = async (id) => {
    ...
    if (confirmation) {
      const url = `https://yarnlink-server.onrender.com/api/mission/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
